...
const SudokuBoard = ({ board, selectedCell, onCellClick }) => 
{
    ...
    if (!board || board.length === 0) return <div className="loading">Loading Board...</div>;
            ...
            {board.map((row, rIndex) => (
                <React.Fragment key={rIndex}>
                    {row.map((cell, cIndex) => 
                    {
                        ...
                        const isSameNumber = selectedCell && selectedValue !== 0 && cell.value === selectedValue && !(selectedCell.r === rIndex && selectedCell.c === cIndex);
